...
import {
    closeConnection, extractTableAndId,
    invalidCookie,
    invalidForm,
    notAuthenticated,
    noUserFound,
...
router.route('/login')
    .post(async (req, res, next) => {
        ...
        try {
            ...
            let personQuery = `
                SELECT person_id as ID, password
                FROM person
                WHERE email = :email
            `
            ...
            if (ret.tableName == 'Management') {
                ...
                query = `
                    ...
                    FROM STUDENT S
                             ...
                             JOIN SECTION S2 on S.SECTION_NAME = S2.SECTION_NAME and S.BATCH_ID = S2.BATCH_ID and
                                                S.DEPARTMENT_ID = S2.DEPARTMENT_ID
